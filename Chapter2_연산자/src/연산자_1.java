 ...
 *                            상수 : 변경할 수 없는 공간
 *             ======================================================== 활용               
 *             배열 : 여러개를 동시에 저장 (같은 형만 저장이 가능) -> 단점 보완한게 클래스
 ...
 *    5) Python : Numpy , Pandas(데이터 관리) , Matplotlib
 *    6) AWS (클라우드) => 호스팅
 ...
 *                  ----------------------------
 *                  byte + byte = int -> 더했을 때 범위가 초과될 수도 있어서
 *                  byte a=127
 *                  byte b=127
 *                  a+b=int => 사용범위 초과
 *            int (4byte) => 프로그램에서 가장 많이 사용되는 데이터형
 *                           정수 => int => dafault
 *                           컴퓨터에서 모든 정수는 int를 인식한다
 *              형식)
 *                 int 변수명=값
 ...
 *                            ---- double (생략)
 *                      float=10.5f(F)
 *                double (8byte) => 실수 가장 많이 사용되는 테이터형
 *                     => default (컴퓨터에서 실수가 있는 경우=> double)
 *                     => 실수정 15자리수까지 인식
 *                *** 정수 < 실수 => 크기가 바이트 크기가 아니라  =>  숫에 대한 표현
 *                   long < float
 *          --------------------------------
 *           문자 : char (2byte) => answksms dmatnrk djqtek (0~65535)
 *                 => 각국의 언어를 표현 => 유니콛, => 호환성이 좋다
 *                 값은 => 문자 '' , 정수값 사용이 가능
 *                  형식)
 *                       char a='A'
 *                       char b=65 ==> 예외 사항
 ...
 *          !b => false ==> 조건 => 로그인, 검색어 ...
 *          
 *        = 형번환 연산자 = 모든 데이터형은 변경이 가능 (boolean 제외)
 ...
 *          || => 병렬연산자 (범위나 기간을 벗어나는 경우) => 위를 벗어났을 때
 ...
 *          int + long = long
 *          ---
 *           | long 변경후에 연산 ==> 데이터형을 큰형으로 변경후에 연산
 *          char + int = int
 *          ----
 *           | int
 *          *** 데이터형의 크기
 ...
 *          
 *          int + double + char = double  -> 가장 큰 데이터형으로 나오는 거
 *          10     10.5     'A'
 *          -----------
 *               |
 *         10.0 + 10.5 
