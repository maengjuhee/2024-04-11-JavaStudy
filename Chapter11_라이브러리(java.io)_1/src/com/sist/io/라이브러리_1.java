 ...
 *      6. BufferedInputStream / BufferedOutputSream
 ...
 *              => 문자 스트림
 *       => movie.txt
 *          FileReader => 한글은 2byte라 이걸로 읽어왔었어
 *          
 ...
 *             -----------------
 *              파일
 *              -- 단점 1. 데이터 노출 / 2. 잃어버릴 가능성 / 3. 관련된 데이터를 따로 저장해서 처리하기 어렵다 /
 *                 4. 구분자가 없다 => 배열에 저장, ArrayList에 저장해서 사용
 *                 -----------------------------------------------------------------------
 *                  => 단점 보완 : RDBMS (관계형 데이터베이스) => 오라클
 *                  => 업로드 / 다운로드
 *                  => 추천 => 카페/블로그 => 읽은 데이터를 파일에 저장 후에 분석 => 분석된 데이터를 오라클로 전송
 *                     => 임시 저장 
 *       2. 파일에 정보 => 376p => 파일 입출력
 *         ---------   -----
 *                       | 377p에 나오는 메소드
