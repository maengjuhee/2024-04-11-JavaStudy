 ...
 *   1. 변수 => 여러개 (배열, 클래스)
 *      => 식별자 : 변수명 설정
 *         ** 대소문자 구분 Name!=name(둘이 다른거야)
 *      => 약속 (문법) 
 *        ------------
 *           리팩토링 
 *        1. 클래스, 인터페이스명은 대문자로 시작한다 (구분하기 위해)
 *           Class Main 
 *           => main main=new main()
 *           ------------------------ 웹에서는 한글이 꺠진다 => 이름을 알파벳 
 ...
 *                  else
 *                  {
 *                      조건이 없는 경우 처리되는 문장 => 필요가 없는 경우에 생략 가능
 *                  }
 *               다중 조건문을 간결하게 만든 제어문
 *               선택문 => switch ~ case (게임에서 많이 쓰고 웹에서는 그리 많이 등장하진 않아)
 *                       게임 => key 입력값
 ...
 *                         for(int i=1;i<=5;i++)
 *                         {
 *                            if(==3) break; ==> 1,2 => 3일때 종료
 *                         }
 *                         for(int i=1;i<=5;i++)
 *                         {
 *                            if(==3) continue; ==> 1,2 => 3일때 증가식으로 올라간다
 *                                     =>1,2,4,5 => 3번을 제외
 *                         }
