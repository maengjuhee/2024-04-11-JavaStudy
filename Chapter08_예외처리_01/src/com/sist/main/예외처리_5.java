 ...
 *                                                        --- 예외처리는 필수
 *     UnCheckedException : 실행시에 예외처리를 처리 하기 때문에 => 생략이 가능하다
 *                          -------------------------- 문법
 *                          
 *     => 예외처리의 종류 (267p)
 ...
 *        SQLException : ******* 핵심 (오라클 실행하는 언어 => SQL)
 *        => Spring / 데이터베이스 가능자
 *           ------ java/jsp/mvc
 *           ---------------------- Back
 *           ---------------------- Front (자바스크립트)
 ...
 *        --------------------------------------------------- 이게 안되면 웹은 포기
 *        => HTML/CSS  : 윈도우 화면 UI (데이터 출력은 없어)       -> 디자인하고 모양 만들러 가라
 *        InterruptedException : 쓰레드 관련
 *        MalformedURLException : 네트워크 관련
 *        ---------------------------------------------------------- CheckedException : 반드시 예외처리
 *        
 *        RumtimeException => 아래있는 모든 예외를 처리할 수 있는 기능이 있다 => 이거 한 개 쓰면 끝
 *        ----------------------------------
 ...
 *                              ---------------------
 *          에외처리의 종류 (자바 뿐만 아니라 모든 프로그램 언어에서 존재, 자바스크립트, 오라클)
 ...
 *                예외처리 : => 소스상에서 수정이 가능(잡으 수 있는 가벼운 에러)
	...
	public static void main(String[] args) {
        ...
        /*try
        {
        	for(int i=1;i<=10;i++)
        	{
        		...
        		System.out.println(i/n); // 오류 발생 => for문을 종료하고 => catch를 찾는다
        	}
        }catch(Exception e)
        {
		 ...
		 *   for문 안에 => 예외처리
		 *   ------------------
		 *   크롤링 => 모든 HTML이 동일하지 않는다 => 스킵 (이렇게 for문 안에 try를 넣어서 해)
		 */
        for(int i=1;i<=10;i++)
        {
        	try
