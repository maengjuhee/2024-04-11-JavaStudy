 ...
 *                                          try~catch
 *                                          --- -----
 *                                              | 에러발생시 처리 => 예상되는 에러를 처리하기 때문에 여러 개 사용 가능
 *                                          |소스 코딩
 *                                 --------------------------------------------------------------------
 *                                 이미 소스 코드가 완료된 상태 => try~catch를 만들기 어렵기 때문에 ...
 *                                 간접처리 : 시스템에 의헤 처리가 가능 => 회피
 *                                         throws
 *                                         ------
 ...
 *                                      
 *                                      
 *                문장 1 => error => 프로그램 종료 => catch는 한번 수행
 *                문장 2 => error       |
 *                문장 3 => 정상수행       | 이 아래 문장은 수행되지 않아
 *                
 */
import java.util.*;
public class 예외처리_3 {
	...
	public static void main(String[] args) {
		...
		try
		{
           ...
           // 첫번째 에러
           int n1=Integer.parseInt(num1); // 문자열을 정수로 변환 ==> 유효성검사 (비밀번호 섞어서 10자리 이상으로 해라 같은 거)
           // => 에러 발생 의심 부분 1
		...
		// 프로그램 정상 종료가 가능하게 만든다 => 윈도우, 브라우저 => 실행상태 유지 ...
