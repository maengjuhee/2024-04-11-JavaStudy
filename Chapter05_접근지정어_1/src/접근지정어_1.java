 ...
 *        Front-End (Java Script) => Ajax, Next, Vue, React
 *       ------------------
 *        데이터 초기화 = 클래스 영역안에는 선언만 가능 => 구현(파일읽기 , 화면 UI, 웹 연결, 연산처리, 제어문
 *          생성자 === 모든 클래스에서 반드시 한 개 이상 포함한다
 ...
 *            = 객체 생성시 반드시 호풀
 *            = 시작과 동시에 
 *          초기화 블록 => 인스턴스변수 => 자동으로 처리 (호출 안 한다)
 *          {
 *              구현이 가능
 *              => 자동 로그인, Cookie , Session 등록, 데이터베이스 연동
 *          }
 *          정적 블록
 *          static
 *          {
 ...
 *             자바 번역)
 *                      new 클래스명()
 *             여러 개의 데이터 / 여러개 메소드가 존재
 *             ----------------------------- . 접근
 *             
 *    => 자바 프로그램의 순서
 *       관련된 클래스를 모아준다 : 패키지
 *       -------------------------client / server => user
 *       => 입출력 => java.io, janax.swing. java.uti
 *       pavkage 경로명;
 ...
 *           클래스 => 기능별로 다시 묶어서 사용
 *                   ----
 *                   게시판 / 회원가입 / 맛집 / 여행 ... 패키지 => 다른 패키지면 같은 이름의 글래스명 사용이 가능
 *                                                ----
 *                                                | 교보서적 
 *                                                | 헌책방
 *                   board
