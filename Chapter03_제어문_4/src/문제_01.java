 ...
 *       switch(op)
 *    => case 처음부터 검색
 *    => 헤딩 문자, 정수를 찾아서 문장을 수행
 *    => case를 찾아서 처리하고  break가 없는 경우에느 break가 존재하는 곳까지 문장을 수행
 *       int a=3;
 *       switch(a)
 *       {
 *         case 1:
 ...
 *          문장4
 *          break;
 *         case 5: ==> 수행 => 문장3 문장4     -> break가 없으면 맨 마지막까지 출력하고 들어가
 *       }
 *       
 *       ** 찾은 case부터 break가 있는 case까지 수행
 *       ** 해당 케이스가 없는 경우에는 default (생략이 가능) => case마다 break;
 *       ** break는 switch~case를 종료
 *       
 *       ** 보통 프로그램에서는 if를 많이 사용한다
 *          경우의 수가 많은 경우엔 switch를 주로 이용한다 (게임, 메뉴)
 *          => switch가 if보다 간결하다
 *          => 네트워크 
 *          
 *       ** if => 범위가 있는 경우
