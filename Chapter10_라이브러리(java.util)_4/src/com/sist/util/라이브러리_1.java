 ...
 *                          ------ 계속 저장만...(순서 없이 => 읽어오는대로 저장)
 *                          ------ 속도가 빠르다
 *              Vector : 동기화
 *                       ---- 저장이 완료가 되면 다음 저장 : 네트워크의 접속자 정보(IP, PORT)
 *                       ---- 속도가 느리다 (끝나야 다음 걸 할 수 있기 때문에) => 보완한 게 ArrayList
...
public class 라이브러리_1 {
	...
	public static void main(String[] args) {
        ...
        map.put("password", "4567"); // 키가 중복되면 => 덮어쓴다 => 중복을 허용하지 않는다 => Cookie
        // 값을 읽어온다
        ...
        Set<String> s=map.keySet(); // 키만 저장 => 중복을 허용하면 안돼서 Set 이용
        for(String ss:s)
        {
        	System.out.println(ss+":"+map.get(ss));
        }
        // Set : 순서가 없다 => 데이터가 순차적으로 들어오지 않아 
        //       => 가급적이면 출력시에 List를 이용한다(순차적)
        // => Map은 클래스 관리
