 ...
 *         |                         | => 배포                           인터프리터
 *                                 AWS => 전체 파일을 모아서 (.war)
 *      프로그래머만 알 수 있는 언어   컴퓨터가 알 수 있는 언이
 *      
 *      컴파일 방식 : 기계어를 변경 => 자바/C/C++/C#
 ...
 *     ------- 한 개 실행파일에서 여러개의 기능을 동시에 수행
 *              -> 게임 (아바타, 비행기)
 *      ----------------------------------------------
 *      1. 주석 : 프로그램과 상관없이코드에 설명을 붙여준다
 *         여러줄 /  ,  한줄 // 
 *      2. 변수 : 메모리에 저장 => 한 개의 데이터마 저장이 가능
 *      3. 메소드 : 사용자가 요청시에 처리하는 기능
 *                ----------- 기능을 분리하는 역할, 다른 클래스와 연결
 *      4. 에약어 : 이미 자바에서 사용하는 단어 (int, double...)
 ...
 *          1) 알파벳이나 한글로 시작 (알파벳 사용시 대소문자 구분)
 ...
 *                   컴퓨터가 인식하는 숫자의 기본 => default(모든 숫자는 ㅑㅜㅅ)
 *                   10(int)
 *            long => 8yte(64bit) => 0,1이 64개만 사용가능
 *                    수 표현이 많이 있다 (금융권, 공기업)
 *           실수
 ...
 *           int i+100;
 *           long l=100L; => L은 long형값이다
 *           char c='문자 1개' => 'A' => '' 오류 발생 => ' ' (공백이라도 줘야해)
 *           double d=10.5D; ==> D는 생략 가능
...
/*
 *    초기화
 *    1) 명시적인 초기화 => int a=10; -> 우리가 임의로 저장하는 것
