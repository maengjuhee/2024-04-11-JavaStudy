...
-----------------------------------
=> 다른 클래스에서도 사용이 가능 => 객체 지향(여러 개 클래스를 만들어서 조립=> 클래스간의 연관관계)
1) 필드(field) => 맴버변수
2) 생성자(constructor)
3) 메소드(method)
------------------------------------ 접근지정어, 옵션 (static final, abstract)
...
1) 객체를 생성하려면 생성자 호출이 반드시 필요한 것은 아니다.  
   => 원래 답은 이건데 최근 들어 new 없이 메모리 할당 가능 => 리플렉션
---------------------------------------------
2) 생성자는 다른 생성자를 호출하기 위해 this()를 사용할 수 있다.
3) 생성자가 선언되지 않으면 컴파일러가 기본 생성자를 추가한다. => 매개 변수가 없는 생성자
...
1) final 필드와 상수는 초기값이 저장되면 값을 변경할 수 없다.v
2) final 필드와 상수는 생성자에서 초기화될 수 있다.
3) 상수의 이름은 대문자로 작성하는 것이 관례이다.v
